...
def get_pile_dataset(tokenizer=None, n_samples=512, block_size=512, local_path=None):
    # Try to load from online source first
    try:
        # First attempt: Load from local path
        if local_path is None:
            # Set default local path if not provided
            local_path = "val.jsonl.zst"
        
        print(f"Attempting to load dataset from local path: {local_path}")
        
        # Verify local file exists
        import os
        if not os.path.exists(local_path):
            raise FileNotFoundError(f"Local dataset file not found at {local_path}")
        
        # Load local dataset
        dataset = load_dataset("json", data_files=local_path, split="train")
        print("Successfully loaded dataset from local source")

    except Exception as local_e:
        # Fallback to HuggingFace Hub if local load fails
        print(f"Failed to load from local source: {local_e}")
        print("Attempting to load dataset from HuggingFace hub...")
        
        try:
            dataset = load_dataset("mit-han-lab/pile-val-backup", split="validation")
            print("Successfully loaded dataset from online source")
        except Exception as hub_e:
            raise RuntimeError(
                f"Failed to load dataset from both local and online sources.\n"
                f"Local error: {local_e}\n"
                f"Online error: {hub_e}"
            )
...
def build_model_and_enc(model_path):
    ...
    # Check if CUDA supports bfloat16
    if torch.cuda.is_bf16_supported():
        torch_dtype = torch.bfloat16
    else:
        torch_dtype = torch.float16
    
    # Create kwargs with conditional dtype
    kwargs = {
